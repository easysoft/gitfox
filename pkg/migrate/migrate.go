...
func (m *Migrator) apply(ctx context.Context, name, version string) error {
	...
	return m.inTransaction(ctx, func(tx *sql.Tx) error {
		...
		if m.driverName == "mysql" {
			for _, query := range bytes.Split(content, []byte(";")) {
				query = removeComments(query)
				if len(query) == 0 {
					continue
				}
				log.Ctx(ctx).Info().Msgf("exec sql: %s", string(query))
				if _, err := tx.ExecContext(ctx, string(query)); err != nil {
					log.Ctx(ctx).Error().Msgf("error running migration %v from %v: %v", version, name, err)
...
func removeComments(in []byte) []byte {
