...
import (
	...
	"github.com/easysoft/gitfox/internal/pipeline/spec/parse/expand"
	"github.com/easysoft/gitfox/internal/pipeline/spec/parse/normalize"
	"github.com/easysoft/gitfox/internal/pipeline/spec/parse/resolver"
	"github.com/easysoft/gitfox/internal/pipeline/spec/parse/walk"
	"github.com/easysoft/gitfox/internal/runner/docker/engine2/compiler"
	...
	"github.com/easysoft/gitfox/internal/runner/docker/engine2/script"
...
func (s *Runner) run(ctx context.Context, stage *drone.Stage, data *client.Context) error {
	...
	envs := environ.Combine(
		...
		data.Build.Params,
		extend.Envs(data.Stage, data.Repo),
	)
	...
	usedSecrets := make(map[string]string)
	...
	ctxExec := extend.WithSecretContext(ctxlogger, usedSecrets)
	if err := s.Exec(ctxExec, spec, state); err != nil {
		log.WithError(err).
