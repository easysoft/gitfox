...
const optionScript = `
if ($Env:GITFOX_NETRC_MACHINE) {
@"
machine $Env:GITFOX_NETRC_MACHINE
login $Env:GITFOX_NETRC_USERNAME
password $Env:GITFOX_NETRC_PASSWORD
"@ > (Join-Path $Env:USERPROFILE '_netrc');
}
[Environment]::SetEnvironmentVariable("GITFOX_NETRC_USERNAME", $null);
[Environment]::SetEnvironmentVariable("GITFOX_NETRC_PASSWORD", $null);
[Environment]::SetEnvironmentVariable("GITFOX_NETRC_USERNAME", $null);
[Environment]::SetEnvironmentVariable("GITFOX_NETRC_PASSWORD", $null);
[Environment]::SetEnvironmentVariable("GITFOX_SCRIPT", $null);
