...
async function generateTypes() {
  const i18nContent = await fs.promises.readFile(path.resolve(process.cwd(), `src/i18n/strings.zh-cn.yaml`), 'utf8')
  const i18nCDEContent = await fs.promises.readFile(
    path.resolve(process.cwd(), `src/cde-gitness/strings/strings.zh-cn.yaml`),
    'utf8'
  )
