...
import mustache from 'mustache' // GITFOX+
import type { EditorDidMount } from 'react-monaco-editor'
import type { editor } from 'monaco-editor'
import type { EditorView } from '@codemirror/view'
import { StringsContextProvider } from '../framework/strings/StringsContextProvider' // GITFOX+
import type { StringKeys, StringsMap } from '../framework/strings/StringsContext' // GITFOX+
...
export const REGEX_VALID_USER_ID = /^[0-9a-zA-Z][0-9a-zA-Z-_.$]*[0-9a-zA-Z]$/
...
export const getString = (key: StringKeys, vars: Record<string, any> = {}) => {
  // GITFOX+BEGIN
  const strings = (StringsContextProvider as unknown as { data: StringsMap }).data
...
export const getErrorMessage = (error: Unknown): string | undefined => {
  ...
  let message = get(error, 'data.error', get(error, 'data.message', get(error, 'message', error)))
  if (typeof message === 'string') {
    try {
      message = getString(`errors.${message.replace(/'/g, "\\'")}` as StringKeys)
