...
const BranchProtectionListing = (props: { activeTab: string }) => {
  ...
  const [curRuleType, setCurRuleType] = useState<boolean>(false)
          ...
          return (
            <Layout.Horizontal spacing="medium" padding={{ left: 'medium' }}>
              <Container onClick={Utils.stopEvent}>
                {row.original.buildin ? (
                  <Text className={css.pillText} font={{ size: 'xsmall' }}>{getString('triggers.internal')}</Text>
                ) : (
                <Popover
                            ...
                            confirmDelete({
                              ...
                              action: async () => {
                                try {
                                  ...
                                  showError(getErrorMessage(exception), 5000, 'failedToDeleteRule')
                                }
                              }
                            })
                          }
  ...
  return (
    <Container>
      ...
      {console.log("curRuleType", curRuleType)}
      {newRule || editRule ? (
        <BranchProtectionForm
          editMode={editRule}
          buildinRule={curRuleType}
          repoMetadata={repoMetadata}
          ruleUid={curRuleName}
            ...
            <>
              <TableV2
                ...
                onRowClick={row => {
                  ...
                  setCurRuleType(row.buildin as boolean)
                  history.push(
                    routes.toCODESettings({
                      repoPath: repoMetadata?.path as string,
                      settingSection: settingSection,
