...
const BranchProtectionForm = (props: {
  ...
  const { editMode = false, buildinRule, repoMetadata, ruleUid, refetchRules, settingSectionMode } = props
            ...
            <Container className={css.main} padding="xlarge">
              <Container className={css.generalContainer}>
                <Layout.Horizontal flex={{ align: 'center-center' }}>
                  ...
                  {console.log('buildinRule', buildinRule, 'editMode', editMode, '')}
                  {(editMode && buildinRule) ? (
                    <FormInput.CheckBox
                    margin={{ top: 'medium', left: 'medium' }}
                    label={getString('triggers.internal')}
                    disabled={true}
                    name="enable"
                  />
                  ):(
