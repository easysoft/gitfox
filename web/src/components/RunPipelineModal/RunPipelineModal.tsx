  ...
  const runPipeline = async (formData: FormData): Promise<void> => {
    ...
    try {
      ...
      if (window.self !== window.parent) {
        window.parent.postMessage({response, type: 'execution-start'}, '*')
      }

      clearToaster()
      showSuccess(getString('pipelines.executionStarted'))
      if (response?.number && !isNaN(response.number)) {
        const execution = response.number.toString();
        if (window.location.href.includes('fullscreen=zentao')) {
          setTimeout(() => {
            history.push(routes.toCODEExecution({ repoPath, pipeline, execution }))
          }, 3000)
        } else {
          history.push(routes.toCODEExecution({ repoPath, pipeline, execution }))
        }
