...
const MergeInfo: React.FC<{
  ...
  const { getString } = useStrings()
  const isZtFlow = pullRequestMetadata.flow
  return (
    <Container className={cx(css.main, css.merged)}>
      <Layout.Horizontal spacing="medium" flex={{ alignItems: 'center' }} className={css.layout}>
        ...
        <Text flex={{ alignItems: 'center' }} className={cx(css.sub, css.merged)}>
          <StringSubstitute
            str={getString(isZtFlow ? 'pr.prMergedBannerInfoFlow' : 'pr.prMergedBannerInfo')}
            vars={{
              user: (
                <Container padding={{ right: 'small' }}>
        ...
        <FlexExpander />
        {(showDeleteBranchButton || (showRestoreBranchButton && !isZtFlow)) && (
          <BranchActionsButton
            {...props}
