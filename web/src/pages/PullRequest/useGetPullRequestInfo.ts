...
import type { TypesListCommitResponse, TypesPullReq, TypesPullReqActivity, TypesPullReqStats, TypesAIRequest } from 'services/code'
...
export function useGetPullRequestInfo() {
  ...
  const [pullReqLastAIReview, setPullReqLastAIReview] = useAtom(pullReqLastAIReviewAtom)
  ...
  useEffect(
    function cleanupAtoms() {
      return () => {
        ...
        setPullReqLastAIReview(undefined)
      }
    },
    [setPullReqMetadata, setPullReqActivities, setPullReqCommits, setPullReqStats, setPullReqLastAIReview]
  )
  ...
  const {
    data: lastAIReview,
    error: lastAIReviewError,
    refetch: refetchAIReview
  } = useGet<TypesAIRequest>({
    path: `/api/v1/repos/${repoMetadata?.path}/+/pullreq/${pullRequestId}/last-aireview`,
    lazy: true
  })
  ...
  useEffect(
    function updatePullReqLastAIReview() {
      if (lastAIReview && !isEqual(lastAIReview.sha, pullReqLastAIReview?.sha)) {
        setPullReqLastAIReview(lastAIReview)
      }
    },
    [lastAIReview, pullReqLastAIReview, setPullReqLastAIReview]
  )
  ...
  const retryOnErrorFunc = useMemo(() => {
    return () =>
      repoError
        ...
        ? refetchAIReview()
        : refetchActivities()
  }, [repoError, refetchRepo, pullReqError, refetchPullReq, refetchActivities, commitsError, refetchCommits, lastAIReviewError, refetchAIReview])
...
const pullReqLastAIReviewAtom = atom<TypesAIRequest | undefined>(undefined)
