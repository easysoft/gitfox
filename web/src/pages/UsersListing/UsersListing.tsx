...
import { TypesUser, useAdminDeleteUser, useAdminListUsers, useUpdateUserAdmin, useUpdateUserBlocked } from 'services/code'
    ...
    () => [
        ...
        Cell: ({ row }: CellProps<TypesUser>) => {
          return (
            <Layout.Horizontal style={{ alignItems: 'center' }}>
              ...
              <Render when={row.original.blocked}>
                <Text font={{ variation: FontVariation.TINY_SEMI }} color={Color.RED_500} className={css.blockedBadge}>
                  {getString('blocked')}
                </Text>
              </Render>
      ...
      },
      {
        ...
        Cell: ({ row }: CellProps<TypesUser>) => {
          ...
          const { mutate: updateBlocked } = useUpdateUserBlocked({ user_uid: row.original.uid || '' })
          ...
          const handleUpdateBlocked = async () =>
            await onConfirmAct({
              action: async () => {
                try {
                  await updateBlocked({ blocked: !row.original.blocked })
                  showSuccess(getString('userUpdateSuccess'))
                  refetch()
                } catch (error) {
                  showError(getErrorMessage(error))
              ...
              message: (
                <Text font={{ variation: FontVariation.BODY2 }}>
                  <StringSubstitute
                    str={getString(
                      row.original.blocked ? 'userManagement.unbanUserMsg' : 'userManagement.banUserMsg',
                      {
                        displayName: row.original.display_name,
                        userId: row.original.uid
                      }
              ...
              intent: 'danger',
              title: getString(row.original.blocked ? 'unbanUser' : 'banUser')
            })
          ...
          return (
            <OptionsMenuButton
              ...
              items={[
                ...
                {
                  text: row.original.blocked ? getString('unbanUser') : getString('banUser'),
                  onClick: () => handleUpdateBlocked()
                },
                {
