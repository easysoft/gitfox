...
import { stringify, parseDocument, type YAMLMap, type Scalar, type YAMLSeq, type Pair } from 'yaml'
          ...
          steps: [
            {
              ...
              type: 'run',
              spec: {
                container: 'hub.zentao.net/library/alpine',
                script: 'echo "hello gitfox"'
              }
            }
          ]
        }
...
const StarterPipelineV0: Record<string, unknown> = {
  ...
  steps: [
    {
      name: 'test',
      image: 'hub.zentao.net/library/alpine',
      commands: ['echo hello gitfox']
    }
  ]
}
...
type AddUpdatePipelineProps = Partial<{
  hideHeader?: boolean
}>

const AddUpdatePipeline = (props: AddUpdatePipelineProps): JSX.Element => {
            ...
            <Layout.Horizontal className={css.layout}>
              ...
              {yamlVersion === YamlVersion.V1
                ? (
                  hideHeader
                    ? (
                      <Container style={{position: 'relative'}} className={cx(css.pluginsContainer, { [css.extendedHeight]: isExistingPipeline })}>
