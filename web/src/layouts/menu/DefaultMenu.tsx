...
import { FingerprintLockCircle, BookmarkBook, UserSquare, Settings, BoxIso } from 'iconoir-react'
...
import { useMembershipSpaces, type SpaceSpaceOutput } from 'services/code'
...
export const DefaultMenu: React.FC = () => {
  ...
  const { space, repoMetadata, gitRef, commitRef } = useGetRepositoryMetadata()
  ...
  const {data: spaces = []} = useMembershipSpaces({queryParams: {query: space}});
  const isSpaceOwner = space && spaces?.some(x => x.space?.identifier === space && x.role === 'space_owner');
      ...
      <Layout.Vertical spacing="small">
        ...
        <Render when={selectedSpace}>
          ...
          <NavMenuItem
            customIcon={<BoxIso />}
            label={getString('artifacts')}
            to={routes.toCODESpaceArtifacts({ space: selectedSpace?.path as string })}
          />
