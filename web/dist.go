...
func Handler() http.HandlerFunc {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/" ||
			...
			w.Header().Set("Cache-Control", fmt.Sprintf("max-age=%d", int(time.Hour.Seconds())*24*7))
		} else if strings.HasSuffix(r.URL.Path, "favicon.ico") {
			w.Header().Set("Cache-Control", fmt.Sprintf("max-age=%d", int(time.Hour.Seconds())*24))
		}
