...
import (
	...
	gittypes "github.com/easysoft/gitfox/git/types"
...
func LoadConfig() (*types.Config, error) {
	...
	if config.Git.Root == "" {
		...
		newPath := filepath.Join(homedir, gitfoxHomeDir)
...
func ProvideDatabaseConfig(config *types.Config) database.Config {
	return database.Config{
		...
		Options:    config.Database.ExtraFlags,
	}
}

// ProvideStorageConfig loads the storage config from the main config.
func ProvideStorageConfig(config *types.Config) storage.Config {
	return storage.Config{
		Local: storage.DriverConfig{
			Driver: storage.DriverFilesystem,
			Parameters: map[string]interface{}{
				"rootdirectory": config.Storage.Local.Directory,
			},
		},
		S3: storage.DriverConfig{},
	}
}
...
func ProvideBlobStoreConfig(config *types.Config) (blob.Config, error) {
	...
	if config.BlobStore.Provider == blob.ProviderFileSystem && config.BlobStore.Bucket == "" {
		...
		config.BlobStore.Bucket = filepath.Join(homedir, gitfoxHomeDir, blobDir)
	}
	return blob.Config{
...
// ProvideArtifactConfig loads the artifact config from the main config.
func ProvideArtifactConfig(config *types.Config) artifact.Config {
	return artifact.Config{
		Storage: artifact.ConfigStorage{
			Provider: config.Artifact.Storage.Provider,
			Prefix:   config.Artifact.Storage.Prefix,
		},
	}
}
