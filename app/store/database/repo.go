...
func (s *RepoStore) GetPublicAccess(ctx context.Context, id int64) (bool, error) {
	stmt := database.Builder.
		...
		From("repositories").Where("repo_id = ?", id)
	...
	var isPublic bool
	if err = db.GetContext(ctx, &isPublic, sql, args...); err != nil {
		return false, database.ProcessSQLErrorf(ctx, err, "Failed executing custom list query")
	}
	return isPublic, nil
}
...
func (s *RepoStore) SetPublicAccess(ctx context.Context, id int64, isPublic bool) error {
	...
	if _, err = db.ExecContext(ctx, sql, args...); err != nil {
		return database.ProcessSQLErrorf(ctx, err, "Failed executing custom list query")
	}
	return nil
}
