...
func (s *RuleStore) FindBuildIn(ctx context.Context, identifier string, repoID int64) (*types.Rule, error) {
	const sqlQuery = ruleSelectBase + `
		WHERE rule_uid = $1 and rule_buildin = true and rule_repo_id = $2`

	db := dbtx.GetAccessor(ctx, s.db)

	dst := &rule{}
	if err := db.GetContext(ctx, dst, sqlQuery, identifier, repoID); err != nil {
		return nil, database.ProcessSQLErrorf(ctx, err, "Failed to find buildin rule")
	}
