...
// botServicePrincipal is the principal representing gitness.
// It is used for all operations executed by gitness itself.
var botServicePrincipal *types.Principal

func NewBotServiceSession() *auth.Session {
	return &auth.Session{
		Principal: *botServicePrincipal,
		Metadata:  &auth.EmptyMetadata{},
	}
}
...
func System(config *types.Config, userCtrl *user.Controller,
	...
	return func(ctx context.Context) error {
		...
		if err := BotService(ctx, config, serviceCtrl); err != nil {
			return fmt.Errorf("failed to setup bot service: %w", err)
		}
...
// BotService sets up the bot service principal that is used for
// resources that are automatically created by the system.
func BotService(
	ctx context.Context,
	config *types.Config,
	...
	svc, err := serviceCtrl.FindNoAuth(ctx, config.Principal.DefaultBot.UID)
	if errors.Is(err, store.ErrResourceNotFound) {
		svc, err = createServicePrincipal(
			ctx,
			serviceCtrl,
			config.Principal.DefaultBot.UID,
			config.Principal.DefaultBot.Email,
			config.Principal.DefaultBot.DisplayName,
			true,
		)
	}
	...
	if !svc.Admin {
		return fmt.Errorf("bot service with uid '%s' exists but is no admin (ID: %d)", svc.UID, svc.ID)
	}

	botServicePrincipal = svc.ToPrincipal()

	log.Ctx(ctx).Info().Msgf("Completed setup of bot service '%s' (id: %d).", svc.UID, svc.ID)
	return nil
}
