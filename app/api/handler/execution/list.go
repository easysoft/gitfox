...
func HandleList(executionCtrl *execution.Controller) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// createdAfter is a unix timestamp
		createdAfter := request.ParseQueryAfterFromRequest(r)
		executions, totalCount, err := executionCtrl.List(ctx, session, repoRef, pipelineIdentifier, pagination, createdAfter)
		if err != nil {
			render.TranslatedUserError(ctx, w, err)
			return
		}
