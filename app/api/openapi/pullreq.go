...
func pullReqOperations(reflector *openapi3.Reflector) {
	...
	getLastAIReview := openapi3.Operation{}
	getLastAIReview.WithTags("pullreq")
	getLastAIReview.WithSummary("获取最后一次AI审查信息")
	getLastAIReview.WithMapOfAnything(map[string]interface{}{"operationId": "getLastAIReview"})
	_ = reflector.SetRequest(&getLastAIReview, new(pullReqRequest), http.MethodGet)
	_ = reflector.SetJSONResponse(&getLastAIReview, new(types.AIRequest), http.StatusOK)
	_ = reflector.SetJSONResponse(&getLastAIReview, new(usererror.Error), http.StatusBadRequest)
	_ = reflector.SetJSONResponse(&getLastAIReview, new(usererror.Error), http.StatusInternalServerError)
	_ = reflector.SetJSONResponse(&getLastAIReview, new(usererror.Error), http.StatusUnauthorized)
	_ = reflector.SetJSONResponse(&getLastAIReview, new(usererror.Error), http.StatusForbidden)
	_ = reflector.Spec.AddOperation(http.MethodGet, "/repos/{repo_ref}/pullreq/{pullreq_number}/last-aireview", getLastAIReview)
