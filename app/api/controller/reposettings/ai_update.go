...
func (c *Controller) AIUpdate(
	...
	if count == 0 {
		return nil, store.ErrRepoConfigAINotFoundProvied
	}

	// read old settings values
	old := GetDefaultAISettings()
	oldMappings := GetAISettingsMappings(old)
	err = c.settings.RepoMap(ctx, repo.ID, oldMappings...)
	if err != nil {
		return nil, fmt.Errorf("failed to map settings (old): %w", err)
	}

	err = c.settings.RepoSetMany(ctx, repo.ID, GetAISettingsAsKeyValues(in)...)
	if err != nil {
