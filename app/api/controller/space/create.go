...
func (c *Controller) createSpaceInnerInTX(
	...
	// added for gitfox artifact
	if err = c.createSpaceView(ctx, space); err != nil {
		return nil, err
	}
	return space, nil
}

func (c *Controller) createSpaceView(ctx context.Context, space *types.Space) error {
	// create default artifact view
	v := types.ArtifactView{
		Name:        "default",
		Description: "",
		SpaceID:     space.ID,
		Default:     true,
	}
	if err := c.artStore.Views().Create(ctx, &v); err != nil {
		log.Ctx(ctx).Err(err).Msg("failed to create view")
		return err
	}
	return nil
}
