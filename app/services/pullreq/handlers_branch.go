...
func (s *Service) triggerPREventOnBranchUpdate(ctx context.Context,
	...
	if branch, err := getBranchFromRef(event.Payload.Ref); err == nil {
		...
		if err != nil {
			log.Ctx(ctx).Err(err).Msgf("failed to reset merge check repo %v (branch: %v ref:%v) status", event.Payload.RepoID, branch, event.Payload.Ref)
			return err
		}
	}
	log.Ctx(ctx).Debug().Msgf("reset merge check status for repo %v (ref: %v)", event.Payload.RepoID, event.Payload.Ref)
	...
	s.forEveryOpenPR(ctx, event.Payload.RepoID, event.Payload.Ref, func(pr *types.PullReq) error {
		...
		pr, err = s.pullreqStore.UpdateOptLock(ctx, pr, func(pr *types.PullReq) error {
			...
			if pr.SourceSHA != event.Payload.OldSHA {
				if pr.Flow != enum.PullRequestFlowZentao {
					return fmt.Errorf(
...
func (s *Service) forEveryOpenPR(ctx context.Context,
	...
	log.Ctx(ctx).Debug().Msgf("forEveryOpenPR: repoID=%d, branch=%s ref=%s", repoID, branch, ref)
