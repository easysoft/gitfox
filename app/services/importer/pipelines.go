...
func convertPipelineFiles(ctx context.Context,
	...
	const (
		myPipelineName     = "pipeline"
		myPipelineNameOnly = "default-" + myPipelineName
		myPipelineDir      = ".gitfox"
		myPipelineFileOnly = myPipelineDir + "/pipeline.yaml"
	)
	...
	for _, file := range files {
		...
		if len(files) == 1 {
			pipelineName = myPipelineNameOnly
			pipelinePath = myPipelineFileOnly
		} else {
			base := path.Base(file.OriginalPath)
			base = strings.TrimSuffix(base, path.Ext(base))
			pipelineName = myPipelineName + "-" + base
			pipelinePath = myPipelineDir + "/" + base + ".yaml"
		}
