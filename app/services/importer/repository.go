...
import (
	...
	gitfoxurl "github.com/easysoft/gitfox/app/url"
...
type Repository struct {
	...
	urlProvider   gitfoxurl.Provider
	git           git.Interface
	tx            dbtx.Transactor
...
func (r *Repository) Handle(ctx context.Context, data string, _ job.ProgressReporter) (string, error) {
	...
	err = func() error {
		...
		if input.Mirror {
			if err := r.repoStore.CreateOrUpdateMirror(ctx, &types.RepositoryMirror{
				RepoID:        repo.ID,
				SpaceID:       repo.ParentID,
				SyncInterval:  1,
				EnablePrune:   true,
				RemoteAddress: input.CloneURL,
			}); err != nil {
				log.Warn().Err(err).Msg("failed to create or update mirror status")
			}
			return nil
		}
