...
func (s *Service) handleEventPullReqCreated(ctx context.Context,
	...
	return s.triggerForEventWithPullReq(ctx, enum.WebhookTriggerPullReqCreated,
		...
		func(principal *types.Principal, pr *types.PullReq, targetRepo, sourceRepo *types.Repository) (any, error) {
			...
			// TODO: add ai review
			log.Ctx(ctx).Info().Msgf("pr create ai review: %v", pr.ID)
			if err := s.aiReviewPullReq(ctx, pr); err != nil {
				log.Ctx(ctx).Error().Msgf("failed to ai review pr: %v", err)
			}
...
func (s *Service) handleEventPullReqBranchUpdated(ctx context.Context,
	...
	return s.triggerForEventWithPullReq(ctx, enum.WebhookTriggerPullReqBranchUpdated,
		...
		func(principal *types.Principal, pr *types.PullReq, targetRepo, sourceRepo *types.Repository) (any, error) {
			...
			// TODO: add ai review
			log.Ctx(ctx).Info().Msgf("pr branch update ai review: %v", pr.ID)
			if err := s.aiReviewPullReq(ctx, pr); err != nil {
				log.Ctx(ctx).Error().Msgf("failed to ai review pr: %v", err)
			}
...
func (s *Service) handleEventPullReqReviewerCreated(
	ctx context.Context,
	event *events.Event[*pullreqevents.ReviewerAddedPayload],
) error {
	return s.triggerForEventWithPullReq(ctx, enum.WebhookTriggerPullReqReviewerCreated,
		event.ID, event.Payload.PrincipalID, event.Payload.PullReqID,
		func(principal *types.Principal, pr *types.PullReq, targetRepo, sourceRepo *types.Repository) (any, error) {
			targetRepoInfo := repositoryInfoFrom(ctx, targetRepo, s.urlProvider)
			...
			return &PullReqReviewerChangedPayload{
				BaseSegment: BaseSegment{
					Trigger:   enum.WebhookTriggerPullReqReviewerCreated,
					Repo:      targetRepoInfo,
					Principal: principalInfoFrom(principal.ToPrincipalInfo()),
				},
				PullReqSegment: PullReqSegment{
				...
				ReviewerSegment: ReviewerSegment{
					Reviewer: principalInfoFrom(reviewer.ToPrincipalInfo()),
				},
			}, nil
		})
}
...
func (s *Service) handleEventPullReqReviewerDeleted(
	ctx context.Context,
	event *events.Event[*pullreqevents.ReviewerDeletedPayload],
) error {
	return s.triggerForEventWithPullReq(ctx, enum.WebhookTriggerPullReqReviewerDeleted,
		event.ID, event.Payload.PrincipalID, event.Payload.PullReqID,
		func(principal *types.Principal, pr *types.PullReq, targetRepo, sourceRepo *types.Repository) (any, error) {
			targetRepoInfo := repositoryInfoFrom(ctx, targetRepo, s.urlProvider)
			reviewer, err := s.principalStore.Find(ctx, event.Payload.ReviewerID)
			...
			return &PullReqReviewerChangedPayload{
				BaseSegment: BaseSegment{
					Trigger:   enum.WebhookTriggerPullReqReviewerDeleted,
					Repo:      targetRepoInfo,
					Principal: principalInfoFrom(principal.ToPrincipalInfo()),
				},
				PullReqSegment: PullReqSegment{
