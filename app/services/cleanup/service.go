...
import (
	...
	artcontroller "github.com/easysoft/gitfox/app/artifact/controller"
...
type Service struct {
	...
	artCtrl               *artcontroller.Controller
}
...
func (s *Service) scheduleRecurringCleanupJobs(ctx context.Context) error {
	...
	err = s.scheduler.AddRecurring(
		ctx,
		artifactgc.JobTypeArtifactContainerGC,
		artifactgc.JobTypeArtifactContainerGC,
		jobCronArtifactContainerGC,
		artifactgc.JobMaxDurationDArtifactContainerGC,
	)
	if err != nil {
		return fmt.Errorf("failed to schedule artifact container GC job: %w", err)
	}

	err = s.scheduler.AddRecurring(
		ctx,
		artifactgc.JobTypeArtifactHardRemove,
		artifactgc.JobTypeArtifactHardRemove,
		jobCronArtifactSoftRemoveGC,
		artifactgc.JobMaxDurationDArtifactSoftRemove,
	)
	if err != nil {
		return fmt.Errorf("failed to schedule artifact hard remove job: %w", err)
	}
	return nil
}
