...
import (
	...
	"github.com/easysoft/gitfox/app/api/handler/account"
	handleraiagent "github.com/easysoft/gitfox/app/api/handler/aiagent"
	"github.com/easysoft/gitfox/app/api/handler/artifact"
	handlercapabilities "github.com/easysoft/gitfox/app/api/handler/capabilities"
	handlercheck "github.com/easysoft/gitfox/app/api/handler/check"
	handlerconnector "github.com/easysoft/gitfox/app/api/handler/connector"
	handlerexecution "github.com/easysoft/gitfox/app/api/handler/execution"
	handlergithook "github.com/easysoft/gitfox/app/api/handler/githook"
	handlergitspace "github.com/easysoft/gitfox/app/api/handler/gitspace"
	handlerinfraProvider "github.com/easysoft/gitfox/app/api/handler/infraprovider"
	handlerkeywordsearch "github.com/easysoft/gitfox/app/api/handler/keywordsearch"
	handlerlogs "github.com/easysoft/gitfox/app/api/handler/logs"
	handlermigrate "github.com/easysoft/gitfox/app/api/handler/migrate"
	handlerpipeline "github.com/easysoft/gitfox/app/api/handler/pipeline"
	handlerplugin "github.com/easysoft/gitfox/app/api/handler/plugin"
	handlerprincipal "github.com/easysoft/gitfox/app/api/handler/principal"
	handlerpullreq "github.com/easysoft/gitfox/app/api/handler/pullreq"
	handlerrepo "github.com/easysoft/gitfox/app/api/handler/repo"
	handlerreposettings "github.com/easysoft/gitfox/app/api/handler/reposettings"
	"github.com/easysoft/gitfox/app/api/handler/resource"
	handlerunner "github.com/easysoft/gitfox/app/api/handler/runner"
	handlersecret "github.com/easysoft/gitfox/app/api/handler/secret"
	handlerserviceaccount "github.com/easysoft/gitfox/app/api/handler/serviceaccount"
	handlerspace "github.com/easysoft/gitfox/app/api/handler/space"
	handlersystem "github.com/easysoft/gitfox/app/api/handler/system"
	handlertemplate "github.com/easysoft/gitfox/app/api/handler/template"
	handlertrigger "github.com/easysoft/gitfox/app/api/handler/trigger"
	handlerupload "github.com/easysoft/gitfox/app/api/handler/upload"
	handleruser "github.com/easysoft/gitfox/app/api/handler/user"
	handlerUserGroup "github.com/easysoft/gitfox/app/api/handler/usergroup"
	"github.com/easysoft/gitfox/app/api/handler/users"
	handlerwebhook "github.com/easysoft/gitfox/app/api/handler/webhook"
	...
	"github.com/easysoft/gitfox/app/api/middleware/nocache"
	middlewareprincipal "github.com/easysoft/gitfox/app/api/middleware/principal"
	"github.com/easysoft/gitfox/app/api/request"
	artctl "github.com/easysoft/gitfox/app/artifact/controller"
	handlerartifact2 "github.com/easysoft/gitfox/app/artifact/handler"
...
func NewAPIHandler(
	...
	r.Route("/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Use(middlewareauthn.Sudo(principalStore))
			...
			setupRoutesV1WithAuth(r, appCtx, config, repoCtrl, repoSettingsCtrl, executionCtrl, triggerCtrl, logCtrl,
				...
				searchCtrl, runnerCtrl, gitspaceCtrl, infraProviderCtrl, migrateCtrl, aiagentCtrl, capabilitiesCtrl)
			setupRouteArtifactV1(r, appCtx, artifactCtrl, spaceCtrl)
		})
	})
...
func setupRoutesV1WithAuth(r chi.Router,
	...
	setupRunner(r, runnerCtrl)
	setupInfraProviders(r, infraProviderCtrl)
	setupGitspaces(r, gitspaceCtrl)
	setupMigrate(r, migrateCtrl)
}

func setupRouteArtifactV1(
	r chi.Router,
	appCtx context.Context,
	artCtrl *artctl.Controller,
	spaceCtrl *space.Controller,
) {
	r.Route(fmt.Sprintf("/artifacts/{%s}", request.PathParamSpaceRef), func(r chi.Router) {
		r.Get("/nodes", artifact.HandListTree(artCtrl))
		r.Post("/nodes/remove", artifact.HandleSoftRemoveByNode(artCtrl))
		r.Route("/raw", func(r chi.Router) {
			r.Post("/upload", handlerartifact2.HandRawUpload(artCtrl))
			r.Get("/versions", artifact.HandListVersions(artCtrl, types.ArtifactRawFormat))
			r.Get("/assets", artifact.HandListAssets(artCtrl, types.ArtifactRawFormat))
		})
		r.Route("/helm", func(r chi.Router) {
			r.Post("/upload", handlerartifact2.HandHelmUpload(artCtrl))
			r.Get("/versions", artifact.HandListVersions(artCtrl, types.ArtifactHelmFormat))
			r.Get("/assets", artifact.HandListAssets(artCtrl, types.ArtifactHelmFormat))
		})
		r.Route("/container", func(r chi.Router) {
			r.Get("/versions", artifact.HandListVersions(artCtrl, types.ArtifactContainerFormat))
			r.Get("/assets", artifact.HandListContainerImages(artCtrl))
		})
	})
	r.Post("/artifacts/nodeInfo", artifact.HandNodeInfos(artCtrl))
	r.Post("/artifacts/refresh_nodes", artifact.HandRefreshTree(artCtrl))
	r.Get("/artifacts/garbage/container", artifact.HandGarbageCollectContainer(artCtrl))
	r.Get("/artifacts/garbage/soft-remove", artifact.HandGarbageCollectSoftRemove(artCtrl))
	r.Get("/artifacts/capacity/container", artifact.HandContainerCapacityStatistic(artCtrl))
	r.Get("/artifacts/capacity/all", artifact.HandCapacityStatistic(artCtrl))
}
...
func setupSpaces(
	...
	r.Route("/spaces", func(r chi.Router) {
		...
		r.Get("/", handlerspace.HandleListAllSpaces(spaceCtrl))
		...
		r.Route(fmt.Sprintf("/{%s}", request.PathParamSpaceRef), func(r chi.Router) {
			...
			r.Get("/artifacts", handlerspace.HandleListArtifacts(spaceCtrl))
...
func SetupSpaceAI(r chi.Router, spaceCtrl *space.Controller) {
	r.Route("/ai", func(r chi.Router) {
		r.Get("/", handlerspace.HandleListAIConfigs(spaceCtrl))
		r.Post("/", handlerspace.HandleCreateAIConfig(spaceCtrl))
		r.Route(fmt.Sprintf("/{%s}", request.PathParamAIConfigID), func(r chi.Router) {
			r.Patch("/", handlerspace.HandleUpdateAIConfig(spaceCtrl))
			r.Delete("/", handlerspace.HandleDeleteAIConfig(spaceCtrl))
			r.Post("/test", handlerspace.HandleTestAIConfig(spaceCtrl))
			r.Post("/default", handlerspace.HandleSetDefaultAIConfig(spaceCtrl))
		})
	})
}
...
func setupRepos(r chi.Router,
	...
	r.Route("/repos", func(r chi.Router) {
		// Create takes path and parentId via body, not uri
		r.Get("/", handlerrepo.HandleListRepos(repoCtrl))
		...
		r.Route(fmt.Sprintf("/{%s}", request.PathParamRepoRef), func(r chi.Router) {
			...
			r.Post("/remove", handlerrepo.HandleHardDelete(repoCtrl))
			...
			r.Route("/settings", func(r chi.Router) {
				...
				r.Get("/ai", handlerreposettings.HandleAIFind(repoSettingsCtrl))
				r.Patch("/ai", handlerreposettings.HandleAIUpdate(repoSettingsCtrl))
			...
			r.Get("/archive", handlerrepo.HandleArchive(repoCtrl))
...
func setupRunner(
	...
	r.Route("/runner", func(r chi.Router) {
		...
		r.Post("/stage", handlerunner.HandleRequest(runnerCtrl))
		r.Route(fmt.Sprintf("/stage/{%s}", request.PathParamStageNumber), func(r chi.Router) {
			r.Post("/", handlerunner.HandleAccept(runnerCtrl))
			r.Get("/", handlerunner.HandleDetails(runnerCtrl))
			r.Put("/", handlerunner.HandleUpdate(runnerCtrl))
		})
		r.Route(fmt.Sprintf("/step/{%s}", request.PathParamStepNumber), func(r chi.Router) {
			r.Put("/", handlerunner.HandleUpdateStep(runnerCtrl))
			r.Post("/logs/batch", handlerunner.HandleBatch(runnerCtrl))
			r.Post("/logs/upload", handlerunner.HandleUpload(runnerCtrl))
		})
		r.Post(fmt.Sprintf("/build/{%s}/watch", request.PathParamExecutionNumber),
			handlerunner.HandleWatch(runnerCtrl))
	})
}
...
func SetupPullReq(r chi.Router, pullreqCtrl *pullreq.Controller) {
	r.Route("/pullreq", func(r chi.Router) {
		...
		r.Route(fmt.Sprintf("/{%s}", request.PathParamPullReqNumber), func(r chi.Router) {
			...
			r.Get("/last-aireview", handlerpullreq.HandleLastAIReview(pullreqCtrl))
	...
	r.Route("/admin", func(r chi.Router) {
		...
		r.Route("/users", func(r chi.Router) {
			...
			r.Route(fmt.Sprintf("/{%s}", request.PathParamUserUID), func(r chi.Router) {
				...
				r.Patch("/blocked", handleruser.HandleUpdateBlocked(userCtrl))
