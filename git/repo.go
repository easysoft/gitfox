...
import (
	...
	"github.com/easysoft/gitfox/git/check"
	"github.com/easysoft/gitfox/git/hash"
...
func (s *Service) SyncRepository(
	...
	log.Ctx(ctx).Debug().Msgf("SyncRepository: git repo synced, mirror: %v", params.Mirror)
	// if mirror set source as remote
	if params.Mirror {
		err = s.git.MirrorAddFetch(ctx, repoPath, "origin", params.Source)
		if err != nil {
			return nil, fmt.Errorf("MirrorRepository: failed to add fetch remote: %w", err)
		}
	}
...
func (s *Service) createRepositoryInternal(
	...
	if err = s.git.Clone(ctx, repoPath, tempDir, api.CloneRepoOptions{Mirror: base.Mirror}); err != nil {
	...
	// 	}
	// }
	if err = s.createDelegateHooks(repoPath); err != nil {
		return errors.Internal(err, "failed to setup createDelegateHooks, gitHookPath: %s", s.gitHookPath)
	}
...
func (s *Service) createDelegateHooks(repoPath string) (err error) {
	// TODO hook bin path s.gitHookPath
	hookNames, hookTpls, giteaHookTpls := util.GetHookTemplates()
	hookDir := filepath.Join(repoPath, "hooks")
	for i, hookName := range hookNames {
		oldHookPath := filepath.Join(hookDir, hookName)
		newHookPath := filepath.Join(hookDir, hookName+".d", "99gitfox")
		...
		if err = util.EnsureExecutable(oldHookPath); err != nil {
		...
		if err = util.EnsureExecutable(newHookPath); err != nil {
			return fmt.Errorf("Unable to set %s executable. Error %v", oldHookPath, err)
		}
	}
	return nil
