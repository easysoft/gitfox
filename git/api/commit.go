...
// CountCommits lists the commits reachable from ref.
func (g *Git) CountCommits(
	ctx context.Context,
	repoPath string,
	ref string,
	...
	cmd := command.New("log",
		command.WithFlag("--pretty=format:'%H %ad %ae'"),
		command.WithFlag("--date=short"),
	...
	commitLine := strings.Split(output.String(), "\n")
	commitStatsMap := make(map[string]map[string]int)
	for _, line := range commitLine {
		if len(line) == 0 {
			continue
		}
		...
		date := parts[1]
		author := strings.Join(parts[2:], " ")
		author = strings.TrimSuffix(author, "'")
		if _, exists := commitStatsMap[date]; !exists {
			commitStatsMap[date] = make(map[string]int)
		}
		commitStatsMap[date][author]++
	}
	var commitStats []types.CommitCount
	for date, authorsMap := range commitStatsMap {
		for author, count := range authorsMap {
			commitStats = append(commitStats, types.CommitCount{Date: date, Author: author, CommitCountData: types.CommitCountData{Commits: count}})
		}
	}
