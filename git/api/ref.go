...
import (
	...
	"github.com/easysoft/gitfox/git/api/foreachref"
...
func (g *Git) WalkReferences(
	...
	go func() {
		cmd := command.New("for-each-ref",
			...
			command.WithFlag("--exclude", "refs/heads/ztflow-*"),
		)
...
func (g *Git) DeleteRef(ctx context.Context, repoPath string, ref string) error {
	if repoPath == "" {
		return ErrRepositoryPathEmpty
	}
