...
import (
	...
	"github.com/easysoft/gitfox/git/diff"
...
func (s *Service) ReceiveDiffFileNames(ctx context.Context, params *DiffParams) (DiffFileNamesOutput, error) {
	...
	repoPath := getFullPathForRepo(s.reposRoot, params.RepoUID)
	fileNames, err := s.git.ReceiveDiffFileNames(
		ctx,
		repoPath,
		params.BaseRef,
		params.HeadRef,
...
func (s *Service) ReceiveDiffSNumStatus(ctx context.Context, params *DiffParams) (CommitNumStatsOutput, error) {
	if err := params.Validate(); err != nil {
		return CommitNumStatsOutput{}, err
	}
	repoPath := getFullPathForRepo(s.reposRoot, params.RepoUID)
	changes, err := s.git.ReceiveDiffSNumStatus(
		ctx,
		repoPath,
		params.BaseRef,
		params.HeadRef,
	...
	if err != nil {
		return CommitNumStatsOutput{}, fmt.Errorf("failed to get diff file data between '%s' and '%s': %w",
			params.BaseRef, params.HeadRef, err)
	}
	return CommitNumStatsOutput{
		Changes: changes,
	}, nil
}
...
func (s *Service) ReceiveCommitNumStat(ctx context.Context, ref string, params *ReadParams) (CommitNumStatsOutput, error) {
	...
	repoPath := getFullPathForRepo(s.reposRoot, params.RepoUID)
	changes, err := s.git.ReceiveCommitNumStat(
		ctx,
		repoPath,
		ref,
		params.AlternateObjectDirs,
	...
	if err != nil {
		return CommitNumStatsOutput{}, fmt.Errorf("failed to get numstat of ref '%s': %w",
			ref, err)
	}
	return CommitNumStatsOutput{
		Changes: changes,
...
func (s *Service) DiffFiles(ctx context.Context, params *DiffFilesParams) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get diff file data between '%s' and '%s': %w",
			params.TargetCommitSHA, params.SourceCommitSHA, err)
	}
	return output, nil
}
